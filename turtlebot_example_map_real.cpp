...
int dim = 5;
int gridSize = dim*res; //Per the previous comment, this value determines the size of the occupancy grid
...
void pose_callback(const geometry_msgs::Pose &msg) { //Callback function for the Position topic (SIMULATION)

    ips_x = msg.position.x - 1; // In the simulation, the point (0,0) is at the center of the grid. I find it easier if we map (0,0) to the bottom left of the grid, which
                                        // is the equivalent transformation of shifting the (-5,-5) to (0,0), which is what is done here
    ips_y = msg.position.y + 4;
    ips_yaw = tf::getYaw(msg.orientation); //The orientation field of the pose topic is a quaternion. The function tf::getYaw converts the quaternion into yaw
}
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "mapping_real");
    ...
    map_metadata.width = dim*res;
    map_metadata.height = dim*res;
    ...
    ros::Subscriber pose_sub = n_map.subscribe("/pose", 1, pose_callback); //Subscribe to IPS from gazebo
    ...
    while (ros::ok()) {
    	...
    	transform.setOrigin(tf::Vector3(-1,4,0));
        ...
        ROS_INFO("ips_x: %f, ips_y: %f",ips_x,ips_y);
    	if(!ranges.empty()){ //Initially, the ranges value is empty, so if that's the case do not enter main body of code
            ...
            for(int i=0;i<angles;i++){ //Loop through all the angles
        		if( !(std::isnan(ranges[i])) && (ranges[i] < cutoff) ){
        		    ...
        		    for(int j=0;j<x.size();j++){//Map bresenham x and y vectors to bresenham results map                     
                        ...
                        if(j == (x.size()-1)){
                            if(!(std::isnan(ranges[i]))){
                                bresenhamResult(j,2) = certainty;  
                            } 
                            else if(std::isnan(ranges[i])){
                                bresenhamResult(j,2) = 1 - certainty;
                            }
                        }
                        else if(j<(x.size()-1)){
                            bresenhamResult(j,2) = 1 - certainty;
